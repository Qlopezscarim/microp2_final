...
#define acc_odr 0b1011
#define acc_conf 0x40
#define cmd_reg 0x7E
#define reset_acc 0x11
#define reset_gyro 0x15
#define reset_mag 0x18

#define gyro_conf BMI160_GYRCONFG_ADDR
/********************************Public Functions***********************************/
...
void BMI160_Init() {
    ...
    BMI160_WriteRegister(cmd_reg,0xB6);
    SysCtlDelay(1600000*2);

    // Power on accelerometer
    BMI160_WriteRegister(cmd_reg,reset_acc);
    BMI160_WriteRegister(acc_conf,acc_odr);
    SysCtlDelay(1600000*2);
    //power on gyro now
    BMI160_WriteRegister(cmd_reg, 0x15);
    return;
}
...
void BMI160_WriteRegister(uint8_t addr, uint8_t data) {
    ...
    data_pair[0] = addr;
    data_pair[1] = data;
    I2C_WriteMultiple(I2C_A_BASE,BMI160_ADDR,data_pair,2);
}
...
uint8_t BMI160_ReadRegister(uint8_t addr) {
    ...
    uint8_t counter = I2C_ReadSingle(I2C_A_BASE,BMI160_ADDR);
    return counter;
